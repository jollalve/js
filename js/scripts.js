...
function verificarMayorDeEdad() {
    ...
    if (edad >= 18) {
        ...
        Swal.fire({
            title: '¡Bienvenido!',
            text: 'Puedes ahora calcular el precio de tu tatuaje',
            icon: 'success',
...
function mostrarOpciones(selectElementId, options) {
...
mostrarOpciones('tamano', sizes);
mostrarOpciones('color', colors);
mostrarOpciones('lugar', locations);
mostrarOpciones('categoria', categories);
...
function ConfiguracionTatuaje(categoria, tamano, color, ubicacion) {
    ...
    this.calcularCostoTotal = function() {
        return this.categoria.cost + this.tamano.cost + this.color.cost + this.ubicacion.cost;
    }
};

function calculateCost() {
    const categoryCost = parseInt(document.getElementById('categoria').value);
    const sizeCost = parseInt(document.getElementById('tamano').value);
    const colorCost = parseInt(document.getElementById('color').value);
    const locationCost = parseInt(document.getElementById('lugar').value);
    totalCost = categoryCost + sizeCost + colorCost + locationCost;
    ...
    fetch('disponibilidad.json')
        ...
        .then(data => {
            ...
            fechaInput.addEventListener('change', function() {
                ...
                const diaSemana = new Date(fechaSeleccionada)
